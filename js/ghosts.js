...
function drawGhost(ghost) {
	...
	eval('drawHelperGhost(ctx,"' + ghost.toUpperCase() + '", GHOST_' + ghost.toUpperCase() + '_POSITION_X, GHOST_' + ghost.toUpperCase() + '_POSITION_Y, GHOST_' + ghost.toUpperCase() + '_DIRECTION, GHOST_' + ghost.toUpperCase() + '_BODY_STATE, GHOST_' + ghost.toUpperCase() + '_STATE, GHOST_' + ghost.toUpperCase() + '_AFFRAID_STATE)');
...
function drawHelperGhost(ctx, ghost, x, y, direction, bodyState, state, affraidState) {
	let imgId = null;
	if(state == 0 || state == 1)
	{
		if(ghost == 'BLINKY') {
		...
		ctx.beginPath();
		ctx.drawImage(document.getElementById(imgId), (x - 12), (y + 16) - 28, 24, 24); 
		ctx.closePath();
		return;
	}
	if(state == -1) {
